...
extension TrackerRepository: TrackerRepositoryProtocol {
    ...
    func addCategory(withId id: UUID, toTracker tracker: Tracker) throws {
        guard 
            let categoryObject = persistencyService.fetchObject(CategoryObject.self, by: \.id, value: id)?.first,
            persistencyService.fetchObject(TrackerObject.self, by: \.id, value: tracker.id)?.first == nil
        else {
