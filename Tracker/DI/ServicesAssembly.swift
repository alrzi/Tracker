...
final class ServicesAssembly: Assembly {
    func assemble(container: Container) {
        ...
        container.register((any AppNotificationManaging).self) { r in
            NotificationModuleFactory.build(
                center: UNUserNotificationCenter.current(),
                providers: [
                    r.resolve(TrackerNotificationProvider.self)!,
                ]
            )
        }
        .inObjectScope(.container)

        container.register(TrackerNotificationProvider.self) { r in
            TrackerNotificationProvider(
