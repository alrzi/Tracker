        ...
        try await persistencyService.perform { context in
            try context.fetchAll(
                FetchRequestBuilder<TrackerObject>()
                    .setPredicates([NotificationDaysPredicate()])
                    .build()
            )
        }
    }
...
private struct NotificationDaysPredicate: PredicateProviding {
    typealias Object = TrackerObject

    var predicate: NSPredicate {
        NSPredicate(format: "%K.@count > 0", #keyPath(TrackerObject.notificationDays))
    }
}
